...
=head1 Command-line Option
  ...
  --sequence <str>      specify the sequence id wanted for drawing
  --startpos <num>      the starting positon on sequence for drawing
  --endpos <num>        the ending positon on seqeunce for drawing
  
  --gene <file>         input file of gene models with exon-intron-structure
  --genemark <str>      display mark for gene models with exon-intron-structure
  --feature <file>      input file of feature in blocks such as transposons, ncRNAs, etc
  --featmark <str>      display mark for feature in blocks such as transposons, ncRNAs, etc

  --fasta <str>         the genome sequence file in fasta format, not necessary, if given, GC content curve will be plotted

  --figurerate <float>  the figure rate, default=0.01, which means 1 dot for 100bp
  --figuredir <str>     specify path to store the result figures
...
foreach my $pre_file (@Gene_file) {
	...
	read_gene_gff($pre_file,\%Gene) if($pre_file=~/\.gff/i);
	push @Gene,\%Gene;
}

foreach my $feature_file (@Feature_file) {
	my %Feature;
	read_feature_gff($feature_file,\%Feature) if($feature_file=~/\.gff/i);
...
`svg2xxx.pl $Fig_dir/$Sequence\_$Startpos\_$Endpos.svg`;
