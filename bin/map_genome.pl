...
=head1 Command-line Option
  ...
  --fasta <str>         the genome sequence file in fasta format, not neccesary, it given, then GC content curve will be plotted
  ...
  --run <str>           set the job parallel type, qsub, or multi, default=multi
...
##run the shell file
if ($Run eq "qsub") {
	`perl $Bin/qsub-sge.pl --inter 10 --maxjob $Cpu $shell_file`;
}
if ($Run eq "multi") {
